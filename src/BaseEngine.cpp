...
{
	std::cout << "BE initialise" << std::endl;
...
{
	std::cout << "BE IO" << std::endl; // You can actually do the following at any time, but you will not often need to do so.
...
int BaseEngine::mainLoop(void)
{
	...
	while (m_iExitWithCode==-1)
	{
		...
		//std::cout << "BE virtMainLoopStartIteration" << std::endl;
		...
		// Hook to allow subclasses to do something here
		//std::cout << "BE virtMainLoopPreUpdate" << std::endl;
		virtMainLoopPreUpdate();

		// Render stuff
		m_iTick = SDL_GetTicks();

		//std::cout << "BE virtCheckForUpdateTime" << std::endl;
		...
		if (virtCheckForUpdateTime()) // If it is time for a change...
		{
			...
			//std::cout << "BE virtMainLoopDoBeforeUpdate" << std::endl;
			virtMainLoopDoBeforeUpdate();
			//std::cout << "BE MainLoop updateAllObjects" << std::endl;
			updateAllObjects(getModifiedTime());
			//std::cout << "BE virtMainLoopDoAfterUpdate" << std::endl;
			virtMainLoopDoAfterUpdate();
		}

		// Hook to allow subclasses to do something here
		//std::cout << "BE virtMainLoopPostUpdate" << std::endl;
		virtMainLoopPostUpdate();

		// Render the screen - only draws if some update handler told it to draw
		//std::cout << "BE virtRenderScreen" << std::endl;
		virtRenderScreen();
	}
	return 0;
}
...
{
	//std::cout << "BE updateAllObjects" << std::endl;
	for (auto it = m_vecDisplayableObjects.begin(); it != m_vecDisplayableObjects.end(); it++)
		if (*it) 
			(*it)->setHasActed(false);
